...
error usart_init(usart *port) {
    ...
    if ( port->usart == USART1) {
        ...
        pin_enable(USART1_RX, GPIO_ALTERNATE); 
        pin_enable(USART1_TX, GPIO_ALTERNATE); 
        pin_select_alternate(USART1_RX, GPIO_AF07);
        pin_select_alternate(USART1_TX, GPIO_AF07);
        RCC->APB2ENR |= RCC_APB2ENR_USART1EN;
        (port->usart)->BRR = usart_compute_div(apb2_freq, port->baud); 
        port->__it_buf = &__buf_usart1;
    } else if (port->usart == USART2) {
        //RCC->AHB1ENR |= RCC_AHB1ENR_GPIOAEN;
        pin_enable(USART2_RX, GPIO_ALTERNATE); 
        pin_enable(USART2_TX, GPIO_ALTERNATE); 
        pin_select_alternate(USART2_RX, GPIO_AF07);
        pin_select_alternate(USART2_TX, GPIO_AF07);
        RCC->APB1ENR |= RCC_APB1ENR_USART2EN;
        (port->usart)->BRR = usart_compute_div(apb1_freq, port->baud); 
        port->__it_buf = &__buf_usart2;
    } else if (port->usart == USART6) {
        //RCC->AHB1ENR |= RCC_AHB1ENR_GPIOAEN;
        pin_enable(USART6_RX, GPIO_ALTERNATE); 
        pin_enable(USART6_TX, GPIO_ALTERNATE); 
        pin_select_alternate(USART6_RX, GPIO_AF07);
        pin_select_alternate(USART6_TX, GPIO_AF07);
