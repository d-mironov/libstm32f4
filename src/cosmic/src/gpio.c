 ...
 */
error gpio_enable(const gpio_pin_t pin, gpio_mode_t mode) {
    if ( pin > PH15 ) {
...
error gpio_init(gpio *gpio) {
    if (gpio_enable(gpio->pin, gpio->mode) != OK) {
        return GPIO_INVALID_SETTING;
    }
    if (gpio_settings(gpio->pin, gpio->speed, gpio->pull_up_down, gpio->push_pull_open_drain) != OK) {
        return GPIO_INVALID_SETTING;
    } 
    return OK;
}
...
error gpio_select_alternate(const gpio_pin_t pin, const u8 af) {
    if ( pin > PH15 ) {
        return GPIO_PIN_TOO_HIGH;
    }
    if (af > 15) {
 ...
 */
error gpio_settings(const gpio_pin_t pin, const u8 speed, const u8 pull_up_down, const u8 push_pull_open_drain) {
    if (pin > PH15) {
        return GPIO_PIN_TOO_HIGH;
 ...
 *
 */
error gpio_set_speed(const gpio_pin_t pin, const u8 speed) {
    GPIO_TypeDef *port = _GPIO_fetch_port(pin);
    if (port == NULL) {
        return GPIO_PIN_TOO_HIGH;
    } else if (speed > GPIO_HIGH_SPEED) {
...
error gpio_set_pull_up_down(const gpio_pin_t pin, const u8 pull_up_down) {
    GPIO_TypeDef *port = _GPIO_fetch_port(pin);
    if (port == NULL) {
        return GPIO_PIN_TOO_HIGH;
 ...
 * @return OK
 */
error gpio_toggle(const gpio_pin_t pin) {
    GPIO_TypeDef *port = _GPIO_fetch_port(pin);
    if (port == NULL) {
        return GPIO_PIN_TOO_HIGH;
    }
 ...
 * @return OK
 */
error gpio_write(const gpio_pin_t pin, const u8 on_off) {
    GPIO_TypeDef *port = _GPIO_fetch_port(pin);
    if (port == NULL) {
        return GPIO_PIN_TOO_HIGH;
    }
 ...
 */
error gpio_high(const gpio_pin_t pin) {
    GPIO_TypeDef *port = _GPIO_fetch_port(pin);
    if (port == NULL) {
        return GPIO_PIN_TOO_HIGH;
    }
 ...
 */
error gpio_low(const gpio_pin_t pin) {
    GPIO_TypeDef *port = _GPIO_fetch_port(pin);
    if (port == NULL) {
        return GPIO_PIN_TOO_HIGH;
    }
 ...
 * @return pin status
 */
u8 gpio_read(const gpio_pin_t pin) {
    GPIO_TypeDef *port = _GPIO_fetch_port(pin);
    if (port == NULL) {
        return 0;
    }
 ...
 */
bool gpio_is_on(const gpio_pin_t pin) {
    GPIO_TypeDef *port = _GPIO_fetch_port(pin);
    if (port == NULL) {
        return 0;
    }
 ...
 */
error gpio_lock(const gpio_pin_t pin) {
    if ( pin > PH15 ) {
        return GPIO_PIN_TOO_HIGH;
    }
    GPIO_TypeDef *port = _GPIO_fetch_port(pin);
